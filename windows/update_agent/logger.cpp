...
bool Logger::Init(const std::wstring& logPath) {
  // Create/open the log file, always append.
  hFile_ = CreateFileW(
    logPath.c_str(),
    GENERIC_WRITE,
    FILE_SHARE_READ,  // Allow agent log to be read while writing.
    nullptr,
    CREATE_ALWAYS,    // Truncate on each agent run (fresh log per update).
...
void Logger::Log(const wchar_t* level, const wchar_t* state, const wchar_t* message) {
  ...
  // Write atomically (single WriteFile call for short lines).
  DWORD written = 0;
