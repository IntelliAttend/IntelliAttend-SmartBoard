...
class TotpEngine {
  ...
  static void _isolateWorker(_TotpIsolatePayload payload) {
    ...
    _generateNextToken(payload, currentSkewMs, secureRandom);
  ...
  static void _generateNextToken(_TotpIsolatePayload payload, int skewMs, Random secureRandom) {
    try {
      ...
      // nonce: 4 random bytes generated by Random.secure() and Base64 encoded.
      final String nonce = base64.encode(
          List<int>.generate(4, (_) => secureRandom.nextInt(256)));
      ...
      // 5. Signature Encoding: 64-char Hexadecimal (full HMAC-SHA256)
      //    16-char truncation removed: full 256-bit HMAC prevents collision
      //    attacks on QR forgery even with millions of captured payloads.
      final String signatureHex = digest.toString();

      // 6. Final Token Assembly: IATT::[Payload]::[Signature]
      // The board glues the transformed data into the exact string that 
      // gets painted as a QR code. 
      // Protocol Header: IATT::
      // Separator: ::
      final String finalToken = 'IATT::$base64Payload::$signatureHex';

      payload.sendPort.send(finalToken);
    } catch (e) {
      // Use print() here — Log.e depends on Flutter's debugPrint which may
      // not be available in the spawned isolate on all platforms.
      // ignore: avoid_print
