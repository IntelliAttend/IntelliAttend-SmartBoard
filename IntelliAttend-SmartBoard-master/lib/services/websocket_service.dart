...
class WebsocketService {
    ...
    try {
      ...
      switch (type) {
        ...
        case 'attendance_closed':
          final closedVersion = message['version'] as int? ?? _sessionState.currentState.version + 1;
          Log.i('[WS] attendance_closed v=$closedVersion');
          _sessionState.applyState(SessionState(
  ...
  void _handleFullStateSync(Map<String, dynamic> message) {
    ...
    // Recover state machine from full_state_sync (critical on WS reconnect)
    final stateStr = message['state'] as String?;
    if (stateStr != null) {
      final version = message['version'] as int? ?? 0;
      _sessionState.applyState(SessionState(
        sessionId: sessionId,
        state: stateStr,
        version: version,
    ...
    Log.i('[WS] full_state_sync: $totalPresent present / $totalStudents total state=$stateStr');
  }
  ...
  void _handleSessionEnded(Map<String, dynamic> message) {
    ...
    _sessionState.applyState(SessionState(
      ...
      state: 'CLOSED',
      version: message['version'] as int? ?? _sessionState.currentState.version + 1,
      presentCount: message['present'] as int? ?? _sessionState.currentState.presentCount,
    ));
