...
// Replacement for ATL CA2W/CW2A to avoid atlstr.h dependency
class CA2W {
public:
    CA2W(const char* s) {
        if (!s) { m_psz = nullptr; return; }
        int len = MultiByteToWideChar(CP_UTF8, 0, s, -1, NULL, 0);
        m_psz = new wchar_t[len];
        MultiByteToWideChar(CP_UTF8, 0, s, -1, m_psz, len);
    }
    ~CA2W() { delete[] m_psz; }
    wchar_t* m_psz;
};
...
public:
    CW2A(const wchar_t* s) {
        ...
        m_psz = new char[len];
        WideCharToMultiByte(CP_UTF8, 0, s, -1, m_psz, len, NULL, NULL);
    }
    ~CW2A() { delete[] m_psz; }
    char* m_psz;
